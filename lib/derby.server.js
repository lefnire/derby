...
function createApp(appModule) {
  ...
  // Script path just needs to be an arbitrary unique name. It's served by Derby middleware, so doesn't actually
  // exist on the server. Was using encodeURIComponent(relativePath), but that breaks Nginx
  view._scriptPath = '/derby/' + relativePath.replace(/\//g,'') + '.js';
